...
#define THIS_FILE   "os_core_nds.c"
...
PJ_DEF(pj_status_t) pj_init(void)
{
	...
	/* Initialisation timer pour la fonction sleep */
	TIMER0_DATA=0; 
    TIMER1_DATA=0; 
...
//    }
    ...
    PJ_LOG(4,(THIS_FILE, "pjlib %s for NDS initialized",
	      PJ_VERSION));
...
unsigned sleep(unsigned msec)
{
   unsigned now; 
   now=timers2ms(TIMER0_DATA, TIMER1_DATA);
   while((unsigned)timers2ms(TIMER0_DATA, TIMER1_DATA)<now+msec) 
    swiDelay(1);
   return 0;
}
...
static pj_status_t init_mutex(pj_mutex_t *mutex, const char *name, int type)
{
	...
	// pthread_mutexattr_settype(), so what follows is a hack
	// until I get RTEMS patched to support the set/get functions.
	PJ_TODO(FIX_NDS_RECURSIVE_MUTEX_TYPE)
	attr.recursive = 1;	
#else
