/* $Id: siprtp.c 1670 2008-01-08 22:50:06Z bennylp $ */
/* 
...
PJ_LOG(3,(THIS_FILE, "pjsip_udp_transport_start"));
...
static void process_incoming_call(pjsip_rx_data *rdata)
{
    ...
    pjsip_tx_data *tdata;
    pj_status_t status;
PJ_LOG(3,(THIS_FILE, "process_incoming_call"));
    /* Find free call slot */
    for (i=0; i<app.max_calls; ++i) {
	if (app.call[i].inv == NULL)
	    break;
...
static pj_bool_t on_rx_request( pjsip_rx_data *rdata )
{
PJ_LOG(3,(THIS_FILE, "on_rx_request"));
...
PJ_LOG(4, (THIS_FILE, "hostname %s", hostname->ptr));    
	pj_sockaddr_in_init(&tmp_addr, hostname, 0);
	addr = pj_inet_ntoa(tmp_addr.sin_addr);
PJ_LOG(4, (THIS_FILE, "hostname %s", addr));    
...
irqEnable(IRQ_VBLANK);
    ...
    status = init_options(my_argc, my_argv);
    if (status != PJ_SUCCESS)
	return 1;
...
//    }
    PJ_LOG(3,(THIS_FILE, "Press A to quit"));
    for (;;) {
       pj_time_val t = {0, 10};
       pjsip_endpt_handle_events(app.sip_endpt, &t);
        swiWaitForVBlank();
