...
const pj_uint16_t PJ_AF_UNIX	= 1/*AF_UNIX*/;
...
const pj_uint16_t PJ_SOCK_RAW	= 0xFFFF/*SOCK_RAW*/;
const pj_uint16_t PJ_SOCK_RDM	= 0xFFFF/*SOCK_RDM*/;
					         ...
					         const pj_str_t *str_addr)
{
	...
	if (addr->sin_addr.s_addr == PJ_INADDR_NONE) {
    	    pj_hostent he;
	    pj_status_t rc;
PJ_LOG(3,(THIS_FILE, "pj_sockaddr_in_set_str_addr %.*s", str_addr->slen, str_addr->ptr));
	    rc = pj_gethostbyname(str_addr, &he);
	    if (rc == 0) {
		addr->sin_addr.s_addr = *(pj_uint32_t*)he.h_addr;
        if (he.h_addr == NULL)
        {
            PJ_LOG(3,(THIS_FILE, "pj_sockaddr_in_set_str_addr NULL"));
        }
	    } else {
